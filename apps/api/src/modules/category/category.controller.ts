...
import { APICreateCategoryDto, APIUpdateCategoryDto, CreateCategoryDto, DeleteCategoryDto, GetUserCategoriesDto, UpdateCategoryDto } from "@spendee-clone/common/dto";
...
export class CategoryController {
  ...
  async createCategory(@Req() req: { user: User }, @Body() createCategoryDto: APICreateCategoryDto) {
    return firstValueFrom(
      this.categoryServiceClient
  ...
  @Authorize()
  @HttpCode(HttpStatus.OK)
  async updateCategory(@Req() req: { user: User }, @Param('id') id: number, @Body() updateCategoryDto: APIUpdateCategoryDto) {
    return firstValueFrom(
      this.categoryServiceClient
        .send<Category, UpdateCategoryDto>({ cmd: 'update-category', role: 'category' }, { ...updateCategoryDto, id, userId: req.user.id })
        .pipe(timeout(5000))
        .pipe(catchError((error) => throwError(() => new RpcException(error))))
    );
  }
  ...
  async deleteCategory(@Req() req: { user: User }, @Param('id') id: number) {
    return firstValueFrom(
      this.categoryServiceClient
        .send<Category, DeleteCategoryDto>({ cmd: 'delete-category', role: 'category' }, { id, userId: req.user.id })
        .pipe(timeout(5000))
        .pipe(catchError((error) => throwError(() => new RpcException(error))))
    );
  }

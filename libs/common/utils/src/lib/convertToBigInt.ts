export const convertToBigInt = (price: string | number, decimal = 2): bigint => {
  const reg = new RegExp('^-?\\d+(?:\\.\\d{0,' + decimal + '})?', 'g');
  const priceStr = (
    typeof price === 'number' || typeof price.toString === 'function' ? price.toString() : price
  ) as string;
  ...
  if (dot === -1) {
    ...
    priceWithDecimal = computedPrice > 0 ? `${truncatedPriceStr}${'0'.repeat(decimal)}` : truncatedPriceStr;
  }
